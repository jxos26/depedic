...
if($_SESSION['user_type']!=2){
  header('Location: login.php?notPermitted=true');
    ...
    if(isset($_POST['forward'])){
      ...
      $sql = "UPDATE documents SET to_office = '$to_office', rcv_office = '' WHERE dms_id=$dms_id;";
      //echo $sql;
      $res=mysqli_query($con, $sql);
    }
            ...
            <table class="table table-striped table-bordered table-hover table-sm" id="dataTable" width="100%">
              ...
              <tbody>
                <?php
                  $con=con();
                  $section = $_SESSION['section'];
                  $sql= "SELECT * FROM `documents` WHERE `rcv_office` = '$section' AND status='Pending' ORDER BY `date_rcv` DESC";
                  $res = mysqli_query($con,$sql);
                  ...
                  while ($row = mysqli_fetch_array($res)) {
                    ...
                    <td>$row[9]</td>
                    <td><a href='view.php?dms_id=$row[0]&status=$row[9]' data-toggle='tooltip' data-placement='left' title='View Details' class='btn btn-success' role='button'><i class='fa fa-fw fa-eye'></i>View</a>
                      <a ";
                      if($section != 'Records') echo "style='display:none;'";
                      echo "href='print.php?dms_id=$row[0]' data-toggle='tooltip' data-placement='left' title='Print Receipt' class='btn btn-primary' role='button'><i class='fa fa-fw fa-print'></i>Print</a>
                    </td>
                    </tr>";
                  }
